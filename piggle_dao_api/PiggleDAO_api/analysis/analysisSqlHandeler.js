...
const selectAnalysisPostList = function (datas){
    return new Promise(async(resolve, reject)=>{
        try {
            ...
            const conditionAnalysisPostRanking = analysisSqlToJson.query.conditionAnalysisPostRanking._text;
            const orderByVotingExpDateDesc = analysisSqlToJson.query.orderByVotingExpDateDesc._text;
            const orderByAnalysisPostRank = analysisSqlToJson.query.orderByAnalysisPostRank._text;
            const paging = analysisSqlToJson.query.paging._text;
            let queryStr

            // 일반글 조회
            if(datas.length == 3) {
                queryStr = selectAnalysisPostList + "\n" + orderByVotingExpDateDesc + "\n" + paging;
            } else {
                // 랭킹글 조회
