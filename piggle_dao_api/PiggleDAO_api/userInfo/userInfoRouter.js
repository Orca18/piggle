...
router.get('/testCreateUserInfo', async function(req,res) {
    return axios.get('http://localhost:5000/userInfo/test',null, null)  
                ...
                digester.digest(data, async function(err, resultPaserSql) {
                    if (err) {
                        ...
                        try{
                            ...
                            // 아직 가입하지 않은 회원
                                // 계정주소로 노바 전송(초기에 0.5노바 지원)
                                const sendResult = await blockchain.sendToAddrByDevAddrWithAmount(addr, 500000);
                                ...
                                const transferResult = await blockchain.transferToken(devMnemonic, addr,tokenId,10000)

                                if(transferResult.code == 500) {
...
router.get('/selectAddrAmountUsingAddr', async function(req,res){  
    try{
        const account = req.query.user_addr;
